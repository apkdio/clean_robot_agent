...
| 多会话与上下文   | 按 session_id 隔离会话，对话持久化到本地（最近6轮），前端可新建/切换历史会话；自由指代（"它怎么样"）自动消解 |
### 1.3 技术栈
...
用户 query
  ...
  → 上下文记录 + 自由指代消解（context_store：记录用户消息；指代词 → 上一轮推荐型号名）
...
### 4.13 tools/context_store.py —— 会话上下文（持久化 + 自由指代消解）
...
| `get_last_models` | 从最近消息倒序找第一条带 models 的 assistant 消息（上一轮推荐型号列表，供指代消解） |
| `ensure_session_id` | 校验前端 UUID，非法则重新生成（防御异常 session_id 写入文件名） |
| `list_sessions` | 列出所有会话（摘要 + 消息数 + 时间），供前端历史对话列表 |

**用途**：
1. **自由指代消解**：用户说"它怎么样/那这个呢"（`REFERENCE_WORDS` 命中）时，agent 用上一轮推荐的第一个型号名替换指代词，替换后的 query 继续正常流程——低成本确定性消解，不调 LLM
2. **历史会话回放**：前端会话抽屉（`/api/sessions`、`/api/sessions/<sid>/messages`）列出/加载历史对话；刷新页面保留上下文

**设计要点**：
- 文件全量保留 + 内存只留 6 轮：文件是持久层（重启可恢复），内存是读热层（避免频繁读文件）
- assistant 消息由 webapp/app.py 在流式完成后写入，并附带 `models`（结构化推荐列表），保证指代消解拿到的是最新推荐
- 写文件追加而非覆写，天然支持多轮累积
...
**背景**：知识库按文件分域（选购指南/故障排除/维护保养/具体型号），"吸力"这类词跨域出现（选购的"吸力参数"、故障的"吸力下降"），全库检索会把跨域词带偏召回。
...
### ADR-17：多会话与上下文用"session_id 隔离 + jsonl 持久化"，自由指代用"型号名替换"
...
- **自由指代用型号名替换而非 LLM 拼接**：承接 ADR-10 的结论——确定性指代（"它/这个/那款" + 上一轮推荐）用代码替换（`REFERENCE_WORDS` + 最近推荐首个型号名），零延迟零幻觉；把历史拼接给 LLM 是另一层能力（语义理解），等真正遇到再上。
...
| 自由指代部分解决 | 指代词（它/这个/那款）已通过型号名替换消解；更复杂的指代（跨话题、多指代）与"历史拼接发送 LLM"仍未做 | 指代消解已实现 |
...
### 7.1 自由指代（指代消解已实现，历史拼接 LLM 待做）

引导式多轮（系统问、用户答，如选购推荐 SOP）已通过 `sops/` 落地；自由指代（"它怎么样""那这个呢"）已通过 `context_store` + 型号名替换消解（见 4.13/ADR-17），零延迟、确定性。

剩余的是把上下文历史拼接发送给 LLM，让"再便宜点的""这个呢"等需要语义理解的指代也有历史语境——方案：生成时拼接最近 1-3 轮历史（bge-m3 支持 8192 token，安全区间 512 token 内），或混合方案（单轮用分类头、多轮用 LLM）。注意：结构化追问（比较新/更便宜）保持代码筛选，不退回 LLM 转述（ADR-10）。
