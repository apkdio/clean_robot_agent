...
| 多轮 SOP 引导 | 选购推荐等场景按标准流程逐步收集需求（预算/宠物），多轮引导 |
...
| 文件 | 职责 |
|------|------|
| `base.py` | 会话状态（内存）+ 执行器：`start_sop`/`continue_sop`/`end_sop`/`match_sop`；知识域定义（`DOMAIN_MAP` + 关键词表）|
...
**关键设计决策**：
- **触发时机**：仅当"选购意图 + 无预算信息"时进 SOP；已含预算的 query（"预算1000以内推荐"）仍走结构化直出，避免行为冲突
- **退出机制**：用户说"算了/退出/取消"时 `end_sop`，回落到正常流程
...
实现要点：
- 上一轮推荐的结构化型号列表（含名称/价格/发布时间）保存在模块级变量，供追问筛选
...
| 自由指代多轮未解决 | "它怎么样""再便宜点的"无法指代上文（引导式多轮已由 SOP 覆盖）| 待历史拼接 |
...
### 7.1 多轮对话上下文（引导式已实现，自由指代待做）

引导式多轮（系统问、用户答，如选购推荐 SOP）已通过 `sops/` 落地。剩余的是自由指代多轮（"它怎么样""再便宜点的"），方案：分类时拼接最近 1-3 轮历史（bge-m3 支持 8192 token，安全区间 512 token 内），或混合方案（单轮用分类头、多轮用 LLM）。
